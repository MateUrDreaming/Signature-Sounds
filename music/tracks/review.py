...
@review_blueprint.route('/review/<int:track_id>', methods=['GET', 'POST'])
def review(track_id: int):
    ...
    if form.validate_on_submit() and sesh:
        ...
        return redirect(url_for('review_bp.review', track_id=track_id))
    ...
    if cursor > 0:
        ...
        prev_review_url = url_for('review_bp.review', track_id=track_id, cursor=cursor - reviews_per_page)
        first_review_url = url_for('review_bp.review', track_id=track_id)
    ...
    if cursor + reviews_per_page < len(reviews):
        ...
        next_review_url = url_for('review_bp.review', track_id=track_id, cursor=cursor + reviews_per_page)
        ...
        last_review_url = url_for('review_bp.review', track_id=track_id, cursor=last_cursor)
    
    #print(services.get_all_reviews(repo.repo_instance))
...
@review_blueprint.route('/review_board/', methods=['GET', 'POST'])
def review_board():
    sesh=False
    user=None
    reviews_per_page = 5
    ...
    cursor = abs((cursor // reviews_per_page) * reviews_per_page)
    
    if cursor > 0:
        # There are preceding articles, so generate URLs for the 'previous' and 'first' navigation buttons.
        prev_review_url = url_for('review_bp.review_board', cursor=cursor - reviews_per_page)
        first_review_url = url_for('review_bp.review_board')

    

    if cursor + reviews_per_page < len(reviews):
        # There are further articles, so generate URLs for the 'next' and 'last' navigation buttons.
        next_review_url = url_for('review_bp.review_board', cursor=cursor + reviews_per_page)
    
        last_cursor = reviews_per_page * int(len(reviews) / reviews_per_page)
        if len(reviews) % reviews_per_page == 0:
            last_cursor -= reviews_per_page
        last_review_url = url_for('review_bp.review_board', cursor=last_cursor)
